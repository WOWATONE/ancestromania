...
const MAPS_CACHE_DIR = 'MapsCache'+DirectorySeparator;
...
var
  ...
  f: TFileStream;
begin
  x := fs_getIniDir+MAPS_CACHE_DIR+MDPrint(MD5String(Url));
...
var
  ...
  f: TFileStream;
begin
  if not DirectoryExists(fs_getIniDir+MAPS_CACHE_DIR) then
    ForceDirectories(fs_getIniDir+MAPS_CACHE_DIR);
  x := fs_getIniDir+MAPS_CACHE_DIR+MDPrint(MD5String(Url));
...
begin
  mv.BeginUpdate;
  MVGLGeoNames.LocationName := Edit1.Text;
  mv.Zoom := 12;
  TrackBar1.Position := mv.Zoom;
  mv.Geolocate;
  mv.EndUpdate;
