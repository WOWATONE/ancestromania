...
constructor TWizardTreeList.Create(AOwner: TComponent);
...
destructor TWizardTreeList.Destroy;
...
procedure TWizardTreeList.SetItems(NewItems: TStrings);
...
procedure TWizardTreeList.SetItemIndex(Value: Integer);
...
procedure TWizardTreeList.AllocateRects;
...
procedure TWizardTreeList.Paint;
...
begin
     ...
     begin
          {
           if (Index > 0) and (Index < Pred(FItems.Count))
            then X := LeftMargin + HorizontalSpace + BoxWidth + HorizontalSpace
           else X := LeftMargin + HorizontalSpace;
            }
           //	if (Index > 0) and (Index < Pred(FItems.Count))
            //then
		...
		Inc(Y, BoxHeight + VerticalSpace * 2);
          ...
          if (Index < Pred(FItems.Count)) then
          begin
	...
	if FItems.Count > 1 then
     begin
		X := LeftMargin + HorizontalSpace + BoxWidth + HorizontalSpace;
		Y := TopMargin + VerticalSpace  + (BoxHeight div 2);
		Canvas.MoveTo(X, Y);

		X := LeftMargin + HorizontalSpace + (BoxWidth div 2);
		Canvas.LineTo(X, Y);

		inc(Y, (( BoxHeight + (VerticalSpace*2))*  (FItems.Count-1)));
		Canvas.LineTo(X, Y);
...
procedure TWizardTreeList.Click;
...
procedure TWizardTreeList.FillBox(Index: Integer; Live: Boolean);
...
procedure TWizardTreeList.SetLineColor(Value: TColor);
...
procedure TWizardTreeList.SetSelectedBoxColor(Value: TColor);
...
procedure TWizardTreeList.SetBoxColor(Value: TColor);
...
procedure TWizardTreeList.SetBoxHeight(Value: Integer);
...
procedure TWizardTreeList.SetBoxWidth(Value: Integer);
...
procedure TWizardTreeList.SetHorizontalSpace(Value: Integer);
...
procedure TWizardTreeList.SetVerticalSpace(Value: Integer);
...
procedure TWizardTreeList.SetTopmargin(Value: Integer);
...
procedure TWizardTreeList.SetLeftMargin(Value: Integer);
...
procedure TWizardTreeList.SetVersion(Value: string);
...
begin
     ...
     RegisterComponents('Perso', [TWizardTreeList]);
